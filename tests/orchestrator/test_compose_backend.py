...
def test_compose_backend_stop_profile_uses_always_profile(tmp_path: Path) -> None:
    calls: list[list[str]] = []
    ...
    assert calls[1][:4] == ["docker", "inspect", "-f", "{{.State.Running}}"]
...
def test_compose_backend_stop_other_gpu_profiles_calls_docker_stop(tmp_path: Path) -> None:
    ...
    compose_stops = [c for c in calls if c[:2] == ["docker", "compose"] and "stop" in c]
    stopped_services = {c[-1] for c in compose_stops}
    assert stopped_services == {"joryu", "joryu-judge"}
    docker_stops = [c for c in calls if c[:2] == ["docker", "stop"]]
    assert {"joryu", "joryu-judge"} == {c[-1] for c in docker_stops}
