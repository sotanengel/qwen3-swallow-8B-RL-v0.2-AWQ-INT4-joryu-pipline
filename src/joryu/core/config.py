...
@dataclass
class CurateJudgeConfig:
    """高品質抽出用 Judge (既定 Llama-Swallow GGUF)。"""
...
class CurateConfig:
    ...
    judge_model: str = "Llama-3.1-Swallow-8B-Instruct-v0.5"
...
def load_config(path: str | Path) -> Config:
    ...
    judge_raw = curate_raw.pop("judge", None) if isinstance(curate_raw, dict) else None
    ...
    if isinstance(judge_raw, dict):
        cfg.curate.judge = _merge_section(cfg.curate.judge, judge_raw)
