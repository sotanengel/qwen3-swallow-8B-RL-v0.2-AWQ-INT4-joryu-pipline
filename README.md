...
- **AWQ-INT4 ローカル推論** (vLLM `vllm serve` OpenAI 互換 + Marlin, RTX 3060 Ti 8GB 想定)
...
```bash
bash scripts/setup-dev.sh   # uv sync + pre-commit 登録（コミット時 lint/format のみ）
bash scripts/check.sh       # 任意: GitHub Actions と同等のローカル検証
```
...
`joryu-up` は git 作業ツリーの差分と、前回起動時の HEAD からのコミット差分（`git pull` 後など）から rebuild 対象を自動判定する。初回起動時は up 対象をすべて build する。**初回または `Dockerfile.vllm-base` 変更時**は `joryu-vllm-base:latest`（torch + git vLLM コンパイル）を先に build する。BuildKit cache (`joryu-uv-cu130`) で wheel DL を再利用する。**Docker Desktop (WSL2) ではメモリ 16GB 以上を推奨** — vLLM コンパイル中に不足するとデーモンが落ちることがある。
...
チャットの Web 検索・天気・URL 取得ツール、および MCP サーバーは環境変数で設定する。
リポジトリ root の [`.env.example`](.env.example) を `.env` にコピーして値を埋める。

| 変数 | 説明 |
|---|---|
| `TAVILY_API_KEY` | Tavily AI Search API キー (未設定時は search スタブ) |
| `JORYU_SEARCH_PROVIDER` | `tavily` (既定) または `stub` |
...
# TAVILY_API_KEY=... を設定 (任意)
uv run joryu-up --detach   # config.yaml で mcp.enabled なら MCP も起動
uv run joryu-mcp --stdio   # 外部 MCP クライアント向け (手動)
```
...
ジョブ非実行時に、styles.yaml の全スタイル（散文 / 短答 / 対話 / レポート）へ同一 prompt を並列投入し、
応答を横並びで比較できる。2 ターン目以降は列ごとに独立した追加質問が可能。

- やり取りは `data/distilled/responses.jsonl` に `category="人間との対話"` で追記（SFT データとして即利用可）
- 蒸留 / 高品質抽出ジョブ実行中は GPU 衝突回避のため送信不可
- 詳細: [docs/chat.md](docs/chat.md)
...
- [docs/chat.md](docs/chat.md) — インタラクティブチャット UI
